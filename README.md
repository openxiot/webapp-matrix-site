...
官网的客户端下载页 `/apps`（中英双语）只读一个数据源：**`src/data/apps/`** 目录。
...
```jsonc
// src/data/apps/ios.json —— 位置已预留，上架后填 version 与 url 即可
{
  "version": "",
  "url": "",                          // 留空 = 页面显示「即将上线」
  "notes": { "zh": [], "en": [] }
...
| 规则 | 说明 |
| --- | --- |
| 文件名 | 即平台标识（`android` / `ios`），与 `src/data/apps/index.ts` 里的映射、页面文案的键一一对应 |
...
1. 上传安装包到对象存储，拿到直链。
2. **整体覆写**自己平台的文件（如 `src/data/apps/android.json`），字段至少含 `version`、`url`、`notes`。
   因为是覆写而非「读—改—写」，不需要解析别人的数据。
3. 提交到本仓库 `main`（建议直接提交，或提 PR 走一次构建预览）。

> 新增平台（比如 HarmonyOS）需要三处改动：`src/data/apps/` 下加数据文件、
